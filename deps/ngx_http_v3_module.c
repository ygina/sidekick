...
static ngx_command_t  ngx_http_v3_commands[] = {
    ...
    { ngx_string("http3_min_ack_delay"),
      NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_CONF_TAKE1,
      ngx_conf_set_num_slot,
      NGX_HTTP_SRV_CONF_OFFSET,
      offsetof(ngx_http_v3_srv_conf_t, min_ack_delay),
      NULL },

    { ngx_string("http3_max_ack_delay"),
      NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_CONF_TAKE1,
      ngx_conf_set_num_slot,
      NGX_HTTP_SRV_CONF_OFFSET,
      offsetof(ngx_http_v3_srv_conf_t, max_ack_delay),
      NULL },
...
ngx_http_v3_create_srv_conf(ngx_conf_t *cf)
{
    ...
    h3scf->min_ack_delay = NGX_CONF_UNSET_UINT;
    h3scf->max_ack_delay = NGX_CONF_UNSET_UINT;
...
ngx_http_v3_merge_srv_conf(ngx_conf_t *cf, void *parent, void *child)
{
    ...
    ngx_conf_merge_uint_value(conf->min_ack_delay,
                              prev->min_ack_delay, 0);

    ngx_conf_merge_uint_value(conf->max_ack_delay,
                              prev->max_ack_delay, 25);
    ...
    quiche_config_set_min_ack_delay(conf->quic.config, conf->min_ack_delay);

    quiche_config_set_max_ack_delay(conf->quic.config, conf->max_ack_delay);
